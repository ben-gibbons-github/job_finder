...
function parseEnvironmentJobs(html) {
    const jobs = [];
    const linkPattern = /<a[^>]+href="((?:https:\/\/environmentjob\.co\.uk)?\/jobs\/[0-9]+-[^"\s]+)"[^>]*>([\s\S]*?)<\/a>/gi;
...
export async function fetchAllEnvironmentJobs() {
    try {
