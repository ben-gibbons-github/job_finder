    ...
    try {
        ...
        for (let page = 0; page < totalPages; page += 1) {
            ...
            for (const hit of hits) {
                ...
                const sourceUrl = typeof hit.url_external === 'string'
                    ? hit.url_external
                    : typeof hit.url === 'string'
                        ? hit.url
                        : '';
                const company = typeof hit.organization_name === 'string'
                ...
                const description = typeof hit.description_short === 'string' ? stripHtmlTags(hit.description_short) : '';
                if (!title || !sourceUrl) {
                    continue;
                }
                normalized.push({
...
export async function fetchAll80KHoursJobs() {
