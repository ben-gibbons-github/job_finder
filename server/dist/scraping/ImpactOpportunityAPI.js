...
function parseImpactOpportunityJobs(html) {
    const jobs = [];
    const linkPattern = /<a[^>]+href="(https:\/\/impactopportunity\.org\/job\/[0-9]+\/[^"\s]+\/)"[^>]*>([\s\S]*?)<\/a>/gi;
...
export async function fetchAllImpactOpportunityJobs() {
    try {
