...
function parseIdealistJobs(html) {
    const jobs = [];
    const linkPattern = /<a[^>]+href="((?:https:\/\/www\.idealist\.org)?\/en\/(?:nonprofit-job|business-job|job)\/[^"\s]+)"[^>]*>([\s\S]*?)<\/a>/gi;
...
export async function fetchAllJobForGoodJobs() {
    try {
        const normalized = await collectPaginatedHtmlJobs({
            ...
            hasNextPage: (html, page) => new RegExp(`/en/jobs\\?page=${page + 1}(?:[^0-9]|$)`, 'i').test(html) || /next page/i.test(html),
        });
