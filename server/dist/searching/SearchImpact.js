...
function normalizeImpactNumber(value) {
...
function containsKeyword(searchableText, keyword) {
...
export function calculateImpactScore(job, shouldLog = false) {
    ...
    const actionBonus = Math.min(SCORE_WEIGHTS.actionHardCap, clamp01(actionHits / SCORE_WEIGHTS.actionHitNormalizer) * SCORE_WEIGHTS.actionBonusScale);
    const diversityBonus = Math.min(SCORE_WEIGHTS.diversityMaxBonus, Math.max(0, matchedCategoryCount - 1) * SCORE_WEIGHTS.diversityPerCategoryBonus);
