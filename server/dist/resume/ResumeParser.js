...
function tokenizeAndBigram(normalizedText) {
    ...
    const words = normalized
        ...
        return TECH_SPELLINGS[lower] || lower;
    })
...
function buildTermWeights(tokens) {
    ...
    const weighted = termsByFreq.map(([term, freq], index) => {
...
function buildAnchors(normalizedText) {
    const sentences = normalizedText
        .split(/[.!?]+/)
        .map((s) => s.trim())
        .filter((s) => s.length > 10);
    const anchors = sentences.map((sentence) => {
        let score = 0;
