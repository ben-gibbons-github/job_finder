...
export function calculateImpactScore(job: ScrapedJob, shouldLog = false): number {
	...
	for (const [categoryName, category] of Object.entries(IMPACT_CATEGORIES) as Array<[CategoryName, ImpactCategory]>) {
		...
		for (const keyword of category.keywords) {
			...
			const partiallyMatchedTokens = keywordTokens.filter((token) =>
				Array.from(searchableTokens).some((candidate) => candidate.includes(token) || token.includes(candidate)),
			).length

			if (
				keywordTokens.length > 0 &&
				keywordTokens.every((token) =>
					Array.from(searchableTokens).some((candidate) => candidate.startsWith(token) || token.startsWith(candidate)),
				)
			) {
				matchedWeight +=
					keyword.weight *
	...
	const preBlendKeywordScore = clamp01(
		categoryBlendScore + actionBonus + diversityBonus + crossDomainBonus - redFlagPenalty - vagueRolePenalty,
	)
	const keywordImpactScore = clamp01(
		preBlendKeywordScore * SCORE_WEIGHTS.keywordImpactBlendWeight +
